...
export class MxRouterError extends Error {
	/**
	 * 错误类型，使用 MxRouterErrorType 枚举来定义具体的错误类型
	...
	/**
	 * 构造函数，用于创建 MxRouterError 实例
	 * @param type 错误类型，使用 MxRouterErrorType 枚举值
	 ...
	 * @returns 一个表示导航中止错误的 MxRouterError 实例
	 */
	 ...
	 * @returns 一个表示导航重定向错误的 MxRouterError 实例
	 */
	 ...
	 * @returns 一个表示导航失败错误的 MxRouterError 实例
	 */
	 ...
	 * @returns 一个表示无效方法错误的 MxRouterError 实例
	 */
