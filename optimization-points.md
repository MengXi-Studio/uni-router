...
### 4. `setCurrentRoute` 每次 `Object.freeze` 的开销
...
### 5. `NavigationFailure.cause` 类型为 `unknown` 过于宽泛
...
### 6. `syncCurrentRoute` 的 `_from` 参数未使用
...
| 优先级 | 编号 | 优化点                               | 类型       |
| ------ | ---- | ------------------------------------ | ---------- |
| P2     | 1    | `back()` 返回值未携带路由信息        | API 设计   |
| P2     | 2    | `isReady` 缺少超时保护               | 健壮性     |
| P3     | 3    | `isSameQuery` 空对象快速路径         | 性能       |
| P3     | 4    | `Object.freeze` 生产模式优化         | 性能       |
| P3     | 5    | `NavigationFailure.cause` 类型宽泛   | 类型安全   |
| P3     | 6    | `syncCurrentRoute` 的 `_from` 未使用 | 代码质量   |
| P3     | 7    | 开发模式专用警告                     | 调试       |
| P3     | 8    | 导航调试模式                         | 调试       |
| P4     | 9    | `params` 页面传参                    | 需额外适配 |
| P4     | 10   | 导航取消机制                         | 需额外适配 |
| P4     | 11   | `UniRouter` 类职责拆分               | 架构优化   |
