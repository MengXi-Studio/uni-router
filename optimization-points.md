...
### 1. ~~`uni.navigateTo` 动画参数未透传~~ ✅ 已实现

**文件**: `src/navigation/navigate.ts`

uni-app 的 `uni.navigateTo` / `uni.redirectTo` / `uni.navigateBack` 支持 `animationType` / `animationDuration` 参数控制页面切换动画（仅 App 端生效）。

**已实现方案（方案 C：A + B 组合）**：

- `RouteLocationPathRaw` / `RouteLocationNamedRaw` 新增可选 `animation` 字段，导航时传入覆盖 meta 默认值
- `RouteMeta` 新增可选 `animation` 字段，作为路由级默认动画
- `back(delta, animation?)` 新增可选 `animation` 参数
- `RouterLink` 组件新增 `animation` prop
- 优先级：`push/replace 调用时传入` > `meta.animation` > `uni 默认值`
- 非 App 平台 animation 参数被 uni API 自动忽略，无需条件编译

### 2. `uni.preloadPage` 分包预加载未支持
...
### 8. `setCurrentRoute` 每次 `Object.freeze` 的开销
...
### 9. `NavigationFailure.cause` 类型为 `unknown` 过于宽泛
...
### 10. `syncCurrentRoute` 的 `_from` 参数未使用
...
| 优先级 | 编号  | 优化点                               | 类型         |
| ------ | ----- | ------------------------------------ | ------------ |
| P1     | 3     | 缺少 `reLaunch` 导航方式             | 功能缺失     |
| P1     | 4     | 缺少 EventChannel 页面通信           | 功能缺失     |
| ~~P2~~ | ~~1~~ | ~~`navigateTo` 动画参数未透传~~ ✅   | ~~功能缺失~~ |
| P2     | 2     | 分包预加载未支持                     | 功能缺失     |
| P2     | 5     | `back()` 返回值未携带路由信息        | API 设计     |
| P2     | 6     | `isReady` 缺少超时保护               | 健壮性       |
| P3     | 7     | `isSameQuery` 空对象快速路径         | 性能         |
| P3     | 8     | `Object.freeze` 生产模式优化         | 性能         |
| P3     | 9     | `NavigationFailure.cause` 类型宽泛   | 类型安全     |
| P3     | 10    | `syncCurrentRoute` 的 `_from` 未使用 | 代码质量     |
| P3     | 11    | 开发模式专用警告                     | 调试         |
| P3     | 12    | 导航调试模式                         | 调试         |
| P4     | 13    | `params` 页面传参                    | 需额外适配   |
| P4     | 14    | 导航取消机制                         | 需额外适配   |
| P4     | 15    | `UniRouter` 类职责拆分               | 架构优化     |
