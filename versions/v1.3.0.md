...
| 版本   | 核心变更                                                         |
| ------ | ---------------------------------------------------------------- |
| v1.0.0 | 路由器核心、命名路由、路由守卫、组合式 API、错误处理、uni 拦截   |
| v1.1.0 | 守卫超时保护、路由变化监听、RouterLink 错误事件                  |
| v1.1.1 | `back()` 守卫修复、`syncRoute()` query 比较、`onUnmount` 兼容    |
| v1.1.2 | `getCurrentPages()` 环境保护、拦截器低版本兼容                   |
| v1.2.0 | 导航动画（NavigationAnimation、meta.animation、RouterLink prop） |
| v1.3.0 | `relaunch` 导航方式、RouterLink `relaunch` prop、reLaunch 拦截   |
...
- **uni API 拦截**：覆盖全部五种导航 API，确保守卫始终生效
