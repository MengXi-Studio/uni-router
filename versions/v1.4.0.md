...
| 字段          | 类型                           | 说明                                   |
| ------------- | ------------------------------ | -------------------------------------- |
| `path`        | `string`                       | 页面路径，需与 pages.json 一致         |
...
| `eventChannel`  | `EventChannel \| undefined` | 页面间通信通道（仅 push）  |
...
| 页面间通信   | 手动管理            | `events` + `eventChannel`                |
...
| 版本   | 核心新增能力                                        |
| ------ | --------------------------------------------------- |
| v1.0.0 | 路由导航、守卫、命名路由、元信息、组合式 API、拦截器 |
| v1.1.0 | 守卫超时保护、路由变化监听、RouterLink @error       |
| v1.2.0 | 导航动画（NavigationAnimation / meta.animation）     |
| v1.3.0 | relaunch 导航方式、RouterLink relaunch prop          |
| v1.4.0 | EventChannel 页面间通信、RouterLink events/@navigated |
