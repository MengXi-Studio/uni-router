...
Uni Router 的路由配置需与 uni-app 的 `pages.json` 保持一致。本节介绍如何手动定义路由配置、创建路由器实例以及路由匹配规则。

::: tip
推荐使用 [`@meng-xi/vite-plugin`](https://github.com/MengXi-Studio/vite-plugin) 从 `pages.json` 自动生成路由配置和类型声明，详见[自动生成路由配置](./auto-generate)。
:::
