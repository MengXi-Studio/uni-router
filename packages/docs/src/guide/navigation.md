...
> 标注 ✅ 的功能需要注册对应插件：`animation` 需要 AnimationPlugin，`events` 需要 ChannelPlugin，`params` 需要 ParamsPlugin
...
```ts
router.back(delta?: number, options?: Record<string, any>): Promise<RouteLocation>
```
...
::: info 需要 ParamsPlugin
`params` 和 `persistent` 功能需要注册 `ParamsPlugin`。未注册时使用将抛出 `PLUGIN_REQUIRED` 错误。
:::
...
::: info 需要 ChannelPlugin
启用 `useUniEventChannel: true` 需要注册 `ChannelPlugin`。未注册时使用将抛出 `PLUGIN_REQUIRED` 错误。
:::
...
::: info 需要 AnimationPlugin
导航动画功能需要注册 `AnimationPlugin`。未注册时使用 `animation` 参数将抛出 `PLUGIN_REQUIRED` 错误。
:::
...
### 2. TabBar 页面用 params 传参（需注册 ParamsPlugin）
