...
- 🧭 **路由导航** — `push()` / `replace()` / `relaunch()` / `back()`，自动识别 TabBar 页面
- 🛡️ **路由守卫** — `beforeEach` / `beforeResolve` / `afterEach` / `beforeEnter`
- 📦 **参数传递** — `params` 支持传递复杂数据（对象、数组等），不暴露在 URL 中，支持持久化存储
- 🔢 **查询参数增强** — `queryInt()` / `queryNumber()` / `queryBool()` 便捷方法，自动解析查询参数为指定类型
- 📋 **路由元信息** — `title` / `isTab` / `requireAuth` 及自定义扩展字段
- 📡 **页面间通信** — `push()` 支持 `events` 参数和 `eventChannel` 返回值，实现页面间双向通信
