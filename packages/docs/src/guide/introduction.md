...
MengXi UniRouter 是一个专为 [uni-app](https://uniapp.dcloud.net.cn/) 设计的路由管理库，提供类似 [vue-router](https://router.vuejs.org/) 风格的 API 体验。
...
MengXi UniRouter 在 uni-app 原生路由 API 之上封装了一层，解决了上述问题，同时保持与 uni-app 静态页面模型（pages.json）的兼容。
...
MengXi UniRouter **不是** vue-router 的完整移植。由于 uni-app 采用静态页面模型（pages.json 声明页面），以下 vue-router 特性**不被支持**：
