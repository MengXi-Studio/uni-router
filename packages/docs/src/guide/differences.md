...
MengXi UniRouter 参考了 vue-router 的 API 设计，但由于 uni-app 框架的特殊性，两者存在重要差异。
...
| vue-router API | MengXi UniRouter | 说明 |
|----------------|-----------------|------|
...
| `<router-link>` | uni-app 使用 `<navigator>` 组件 |
| 嵌套路由 | uni-app 无嵌套视图 |
| 命名视图 | uni-app 无多视图支持 |
...
vue-router 中 TabBar 页面与普通页面导航方式相同。MengXi UniRouter 中需要根据 `meta.isTab` 自动选择不同的 uni API：

```ts
// vue-router：统一使用 push
router.push('/user')

// MengXi UniRouter：根据 isTab 自动选择
...
vue-router 操作浏览器 History 栈。MengXi UniRouter 操作 uni-app 页面栈：
...
vue-router 中 replace 仅替换当前路由记录。MengXi UniRouter 中 replace 到 TabBar 页面会关闭所有非 Tab 页面（由 `uni.switchTab` 行为决定）。

## 守卫差异

### next() 行为

两者的 `next()` 行为基本一致，但 MengXi UniRouter 的重定向深度限制为 10 次。

### 错误处理

vue-router 4.x 中守卫抛出错误会导致导航失败。MengXi UniRouter 行为一致，守卫异常会被视为 `NAVIGATION_CANCELLED`。

## 迁移建议

如果你从 vue-router 迁移到 MengXi UniRouter：

1. **移除 `<router-view>` 和 `<router-link>`**：uni-app 不需要这些组件
2. **移除 `router.go()` 和 `router.forward()`**：使用 `router.back()` 替代
3. **移除动态路由注册**：所有页面必须在 `pages.json` 中声明
4. **为 TabBar 页面设置 `isTab: true`**：确保使用正确的导航 API
5. **调整 replace 到 TabBar 页面的逻辑**：注意 `switchTab` 会关闭所有非 Tab 页面
6. **移除嵌套路由和命名视图**：uni-app 不支持
