...
MengXi UniRouter references vue-router's API design, but there are important differences due to uni-app framework specifics.

## Design Philosophy

| | vue-router | MengXi UniRouter |
|---|-----------|-----------------|
...
| vue-router API | MengXi UniRouter | Notes |
|----------------|-----------------|-------|
...
| `<router-link>` | uni-app uses `<navigator>` component |
...
In vue-router, TabBar pages use the same navigation method as regular pages. In MengXi UniRouter, different uni APIs are automatically selected based on `meta.isTab`:

```ts
// vue-router: unified push
router.push('/user')

// MengXi UniRouter: auto-select based on isTab
...
vue-router manipulates the browser History stack. MengXi UniRouter manipulates the uni-app page stack:
...
In vue-router, replace only replaces the current route record. In MengXi UniRouter, replacing to a TabBar page closes all non-tab pages (determined by `uni.switchTab` behavior).

## Guard Differences

### next() Behavior

The `next()` behavior is mostly the same, but MengXi UniRouter has a redirect depth limit of 10.

### Error Handling

In vue-router 4.x, guards throwing errors cause navigation to fail. MengXi UniRouter behaves the same — guard exceptions are treated as `NAVIGATION_CANCELLED`.

## Migration Guide

If you're migrating from vue-router to MengXi UniRouter:

1. **Remove `<router-view>` and `<router-link>`**: uni-app doesn't need these components
2. **Remove `router.go()` and `router.forward()`**: Use `router.back()` instead
3. **Remove dynamic route registration**: All pages must be declared in `pages.json`
4. **Set `isTab: true` for TabBar pages**: Ensure the correct navigation API is used
5. **Adjust replace-to-TabBar logic**: Note that `switchTab` closes all non-tab pages
6. **Remove nested routes and named views**: uni-app doesn't support them
