...
Uni Router's route configuration must be consistent with uni-app's `pages.json`. This section covers how to manually define route configurations, create a router instance, and route matching rules.

::: tip
It's recommended to use [`@meng-xi/vite-plugin`](https://github.com/MengXi-Studio/vite-plugin) to auto-generate route configurations and type declarations from `pages.json`. See [Auto-Generating Route Configuration](./auto-generate).
:::
